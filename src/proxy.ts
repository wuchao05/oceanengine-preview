...
const proxyOptions = {
  ...
  onProxyReq: (proxyReq: any, req: Request, res: Response) => {
    ...
    if (originalHeaders["content-type"]) {
      proxyReq.setHeader(
        "Content-Type",
        originalHeaders["content-type"] as string
      );
    }
    if (originalHeaders["cookie"]) {
      proxyReq.setHeader("Cookie", originalHeaders["cookie"] as string);
    }
    if (originalHeaders["user-agent"]) {
      proxyReq.setHeader("User-Agent", originalHeaders["user-agent"] as string);
    } else {
      proxyReq.setHeader(
        "User-Agent",
  ...
  onProxyRes: (proxyRes: any, req: Request, res: Response) => {
    ...
    res.end = function (chunk?: any, encoding?: any) {
      ...
      try {
        ...
        console.log(
          `[PROXY RESPONSE] Body (buffer):`,
          responseBody.toString("hex").substring(0, 200)
        );
      }
...
app.listen(PORT, () => {
  ...
  console.log(
    `[PROXY] 使用方式: 将请求地址改为 http://localhost:${PORT}/api/proxy/...`
  );
});
